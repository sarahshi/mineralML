...
def train_nn_optim(model, optimizer, label, train_loader, test_loader, n_epoch, criterion, patience, trial):
    """
    Train a neural network using early stopping.
    ...
    for epoch in range(n_epoch):
        ...
        # Check if Optuna suggests stopping the training
...
def nested_cross_val(df, name, hidden_layer_large, hidden_layer_small, learning_rates, weight_decays, epochs, n_trials=10):

    reports_dict = {}  # Create a dictionary to store train and test reports for each trial

    def objective(trial):
        hll = trial.suggest_int("hll", hidden_layer_large[0], hidden_layer_large[1], 2)
        hls = trial.suggest_int("hls", hidden_layer_small[0], hidden_layer_small[1], 2)
        lr = trial.suggest_loguniform("lr", learning_rates[0], learning_rates[1])
        wd = trial.suggest_loguniform("wd", weight_decays[0], weight_decays[1])
        ep = trial.suggest_int("ep", epochs[0], epochs[1], 5)

        avg_train_report, avg_test_report = nested_cross_val_opt(df, name, hll, hls, lr, wd, ep, trial)

        if trial.should_prune():
            raise optuna.exceptions.TrialPruned()

        reports_dict[trial.number] = {
        ...
        return -avg_test_report['fscore'].mean()

    study = optuna.create_study()
    study.optimize(objective, n_trials=n_trials)
    best_hidden_layer_large = study.best_params["hll"]
    best_hidden_layer_small = study.best_params["hls"]
    best_learning_rate = study.best_params["lr"]
    best_weight_decay = study.best_params["wd"]
    best_epochs = study.best_params["ep"]
    best_params = np.array([best_hidden_layer_large, best_hidden_layer_small, best_learning_rate, best_weight_decay, best_epochs])

    return reports_dict, best_params


def nested_cross_val_opt(df, name, hidden_layer_large, hidden_layer_small, lr, wd, ep, trial):

    outer_cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    oxides = ['SiO2', 'TiO2', 'Al2O3', 'FeOt', 'MnO', 'MgO', 'CaO', 'Na2O', 'K2O', 'Cr2O3']
    label = ['Mineral']

    X, y = df[oxides].fillna(0).to_numpy(), pd.Categorical(df['Mineral']).codes
    ...
    for train_idx, test_idx in outer_cv.split(X, y):
        ...
        train_pred_classes, test_pred_classes, train_report, test_report = neuralnetwork_kfold_optuna(df, X_train, y_train, X_test, y_test, name, hidden_layer_large, hidden_layer_small, lr, wd, ep, trial)
    ...
    return avg_train_report, avg_test_report


def neuralnetwork_kfold_optuna(df, X_train, y_train, X_test, y_test, name, hidden_layer_large, hidden_layer_small, learning_rate, weight_decay, epochs, trial): 
    ss = StandardScaler()
...
name = "nn_kfold_testing_optuna"

hidden_layer_large = (32, 256)# [(64, 32), (128, 64)]
hidden_layer_small = (16, 128)# [(64, 32), (128, 64)]

learning_rates = (1e-5, 1e-1)
weight_decays = (1e-5, 1e-1)
epochs = (200, 300)
n_splits_outer = 5
n_splits_inner = 3
n_trials = 200
...
best_hidden_layers, best_learning_rate, best_weight_decay, best_epochs = nested_cross_val_optuna(min_df, name, hidden_layer_large, hidden_layer_small, learning_rates, weight_decays, epochs, n_trials)
...
avg_train_report, avg_test_report, best_params = nested_cross_val(min_df, name, hidden_layer_sizes, learning_rates, weight_decays, epochs, n_splits_outer)
...
avg_train_report, avg_test_report, best_params = nested_cross_val(min_df, name, hidden_layer_sizes, learning_rates, weight_decays, epochs, n_splits_outer, n_splits_inner)
print("Average Train Report:")
print(avg_train_report)
print("\nAverage Test Report:")
print(avg_test_report)
...
# names = ["mindf_nn_64_16_4_noP", "mindf_nn_128_32_8_noP"]
# nodes_list = [(64, 16, 4), (128, 32, 8), ]

# for i in range(len(names)): 
#     start_time = time.time()
#     print("starting " + str(names[i]))
#     autoencode(min_df, names[i], nodes_list[i]) 
