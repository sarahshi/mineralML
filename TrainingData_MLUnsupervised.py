...
train_pred_classes, test_pred_classes, train_report, test_report = neuralnetwork(min_df, name, MultiClassClassifier, hls, lr, wd, dr, epochs, n, balanced = True) 
...
# model = OneLayerClassifier(input_dim=input_size).to(device) 
...
# feature_dataset = FeatureDataset_nn(lepr_norm_wt, pd.Categorical(lepr_df['Mineral']).codes)
# input_size = len(feature_dataset.__getitem__(0)[0])
...
# model = MultiClassClassifier(input_dim=input_size, hidden_layer_sizes=hls).to(device) # dropout_rate = dr
# optimizer = torch.optim.Adam(model.parameters(), lr=5e-4, weight_decay=0)
# load_model(model, optimizer, path)

# # new_classes = 15  # Replace with the correct number of classes for your new dataset
# # modified_model = MultiClassClassifierModified(pretrained_model=model, new_classes=new_classes).to(device)

# # for param in modified_model.encode[:-1].parameters():
# #     param.requires_grad = False

# valid_pred_classes = modified_model.predict(feature_dataset.x)

# unique_classes = np.unique(np.concatenate([pd.Categorical(lepr_df['Mineral']).codes, valid_pred_classes]))
# mapping = dict(zip(unique_classes, pd.Categorical(lepr_df['Mineral']).categories[unique_classes]))
# sort_dictionary = dict(sorted(mapping.items(), key=lambda item: item[0]))

# cm_valid = confusion_matrix(pd.Categorical(lepr_df['Mineral']).codes, valid_pred_classes)

# df_valid_cm = pd.DataFrame(cm_valid, index=sort_dictionary.values(), columns=sort_dictionary.values())
