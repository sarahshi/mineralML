...
def autoencode(name, AE_Model, hidden_layer_sizes):
    ...
    for i in range(len(phase)):
        ...
        ax.scatter(z[indx, 0], z[indx, 1], s=15, color=scalarMap.to_rgba(i), alpha = 0.5, lw=1, label=phase[i])
...
z = autoencode(names[i], Tanh_Autoencoder, (512, 128, 32, 8)) # (64, 32)) # (512, 128, 32, 8)
...
for i in range(len(phase)):
    indx = LEPR_AllPhases['Phase'] == phase[i]
    ax_scatter.scatter(z[indx, 0], z[indx, 1], s=15, color=scalarMap.to_rgba(i), alpha = 0.5, lw=1, label=phase[i])
ax_scatter.set_xlabel("Latent Variable 1")
ax_scatter.set_ylabel("Latent Variable 2")
ax_scatter.set_xlim([-1, 1.5])
ax_scatter.set_ylim([-1, 2.0])

pc1_sns = sns.kdeplot(data = z_scores_df, x = 'LV1', color = 'k', ax = ax_hist_x)
pc1_sns.set_xlim([-1, 1.5])
pc1_sns.set(xlabel = None)

pc2_sns = sns.kdeplot(data = z_scores_df, y = 'LV2', color = 'k')
pc2_sns.set_ylim([-1, 2.0])
pc2_sns.set(ylabel = None)

plt.tight_layout()
# plt.savefig(path_grandparent + output_dir[0] + "/" + name + "_tanhauto.pdf")


# %% 

cpxamph = ['Clinopyroxene', 'Amphibole']
...
for i in range(len(cpxamph)):
    indx = LEPR_AllPhases['Phase'] == cpxamph[i]
    ax_scatter.scatter(z[indx, 0], z[indx, 1], s=15, color=scalarMap.to_rgba(i), alpha = 0.5, lw=1, label=phase[i])
ax_scatter.set_xlabel("Latent Variable 1")
ax_scatter.set_ylabel("Latent Variable 2")
# ax_scatter.set_xlim([-1, 1.5])
